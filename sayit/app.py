...
from sayit.audio import AudioCapture
from sayit.stt import FasterWhisperBackend, GroqWhisperBackend, NullBackend
from sayit.clean import RuleCleaner
from sayit.inject import paste_text
from sayit import history, sound, models
...
class SayItApp:
        ...
        with self._lock:
            if self.state is State.RECORDING:
                try:
                    ...
                    log.warning("[sayit] audio stop during cancel: %s", exc)
        ...
        with self._lock:
            if self.state in (State.IDLE, State.ERROR):
                ...
                try:
                    ...
                    log.exception("[sayit] audio start failed")
    ...
    def _process(self, rid):
        ...
        try:
            ...
            if self._run_seq != rid:
                return
            log.exception("[sayit] processing failed; utterance discarded")
    ...
    def run(self):
        ...
        from sayit.ui import SayItUI

        self._listener = HotkeyListener(self.config.hotkey, self.toggle)
        self._listener.start()
        msg = f"[sayit] ready — press {self.config.hotkey} to dictate (Ctrl+C to quit)"
        log.info(msg)
        print(msg)
        try:
            SayItUI(self).run()  # blocks on mainloop; KeyboardInterrupt propagates out
        finally:
            try:
                self.audio.stop()  # release the mic if we tore down mid-recording (BUG 2)
            except Exception:
                log.exception("[sayit] audio.stop() on shutdown failed")
            self._listener.stop()
    ...
    def set_hotkey(self, new_hotkey: str) -> None:
        ...
        try:
            ...
            log.exception("[sayit] set_hotkey failed; restoring previous binding")
            self.last_error = repr(exc)
            self._listener = old
            if old is not None:
                try:
                    ...
                    log.exception("[sayit] could not restore previous hotkey")
    ...
    def _build_stt(self):
        """Build the STT backend from config.stt_provider.
        Nothing loads here (FasterWhisperBackend is lazy), no GPU, no network:
          groq  -> cloud GroqWhisperBackend if key present, else local if downloaded, else NullBackend.
          local -> FasterWhisperBackend only if the model is already downloaded, else NullBackend.
        """
        ...
        if provider == "groq":
            ...
            elif models.is_downloaded(self.config.model):
    ...
    def set_stt_provider(self, provider: str) -> None:
        ...
        try:
            ...
            log.exception("[sayit] set_stt_provider failed")
            self.last_error = repr(exc)
    ...
    def set_groq_model(self, model: str) -> None:
        ...
        try:
            ...
            log.exception("[sayit] set_groq_model failed")
            self.last_error = repr(exc)
    ...
    def set_model(self, name: str) -> None:
        ...
        try:
            ...
            log.exception("[sayit] set_model failed")
            self.last_error = repr(exc)
