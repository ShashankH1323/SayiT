...
def record(text: str, max_items: int, path: str | Path = _DEFAULT_PATH) -> None:
    ...
    try:
        ...
        log.exception("[sayit] history.record failed; transcript not saved")
...
def delete_record(ts: str, text: str, path: str | Path = _DEFAULT_PATH) -> bool:
    ...
    try:
        ...
        log.exception("[sayit] history.delete_record failed")
    return False
...
def clear_all(path: str | Path = _DEFAULT_PATH) -> None:
    ...
    try:
        ...
        log.exception("[sayit] history.clear_all failed")
...
def demo() -> None:
    ...
    tmp = Path(tempfile.gettempdir()) / "sayit_history_selfcheck.jsonl"
    ...
    try:
        ...
        missing = Path(tempfile.gettempdir()) / "sayit_history_nope.jsonl"
